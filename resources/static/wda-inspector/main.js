...
module.exports = "{{#hasChildren}}<a id=\"{{num}}\" href=\"#\"><i class=\"glyphicon glyphicon-minus element-with-children\"></i></a>{{/hasChildren}}<span class=\"label label-default el-type\" data-rect=\"{{rectStr}}\"><strong>XCUIElementType{{type}}</strong></span>{{#rawIdentifier}}<span class=\"el-id label label-success\">{{rawIdentifier}}</span>{{/rawIdentifier}}<span class=\"el-label\">{{label}}</span>";
